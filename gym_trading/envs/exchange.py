...
class BaseExchange(Exchange):
    ...
    def __init__(
        ...
        self.wallet = {asset: 0.0 for asset in self.charts.keys()}
    ...
    def market_buy(self, asset: str, amount: Decimal, date: datetime):
        ...
        price = self.charts[asset].price_at(date)
    ...
    def equities(self) -> Tuple[List[datetime], List[float]]:
        ...
        return self.equity_history['Date'].tolist(), self.equity_history['Equity'].tolist()

    def budget_distribution(self, date: datetime) -> np.ndarray:
        total_equity = self._equity_at(date)

        distribution = np.zeros(len(self.charts.keys()))
    ...
    @staticmethod
    def _add_percentage(value, percentage):
        """
    ...
    def _update_equities_history(self, date: datetime):
        ...
        if not (self.equity_history['Date'] == date).any():
            ...
            self.equity_history.loc[self.equity_history['Date'] == date, 'Equity'] = current_equity

    def _equity_at(self, date: datetime) -> float:
        current_equity = self.liquidity

        for asset, asset_amount in self.wallet.items():
            price = self.charts[asset].price_at(date)
            current_equity += self._add_percentage(asset_amount * price, -self.sell_fee)
